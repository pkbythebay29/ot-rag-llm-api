...
def rebuild_index_in_worker(system_name: str, timeout_sec: float = 900.0) -> dict[str, Any]:
    global _executor
    ...
    try:
        ...
        _status["last_finished_at"] = time.time()
        raise RuntimeError(f"Index rebuild worker failed for '{system_name}': {exc}") from exc
